...
public class Frequency_float {
	...
	public static void init(String features, int clusterNum, String clusters){
		Frequency_float.clusterNum = clusterNum;
		Frequency_float.clusters = clusters;
		Frequency_float.features = features;
	}
	...
	public static void runFreMR(String infile, String outfile){
		...
		JobConf conf = new JobConf(Frequency_float.class);
	    ...
	    System.out.println("!!!!!!"+features + clusterNum + clusters);
	...
	public static class FreMap extends MapReduceBase implements Mapper<Text, VectorWritable, Text, Text> {
		...
		public static float[][] readClusters(String clusters, int clusterNum) throws IOException{
			Configuration conf = new Configuration();
			FileSystem fs = FileSystem.get(conf);
			Path path = new Path(clusters);
			FSDataInputStream input = fs.open(path);
			float[][] cs = new float[clusterNum][featureSize];
			String line;
			//System.out.println(clusters);
			while((line = input.readLine()) != null){
				...
				if(center.contains(":") == false) {
					for(int j = 0; j < featureSize; j++){
						...
						cs[i][j] = Float.parseFloat(co[co.length - 1]);
					}
				}
					...
					for(int j = 0; j < featureSize; j++){
						cs[i][j] = Float.parseFloat(result_array[j]);
					}
				}
			}
			return cs;
		...
		public static int findBestCluster(double[] feature, float[][] clusters){
