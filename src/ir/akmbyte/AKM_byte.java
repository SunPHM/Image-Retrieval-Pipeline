...
//
	...
	private void runIteration(String input_dataset, String clusters_in, String clusters_out)
			...
			job.setJarByClass(AKM_byte.class);
			job.setMapperClass(AKM_byte.AKM_Mapper.class);
			job.setReducerClass(AKM_byte.AKM_Reducer.class);
	...
	public static class AKM_Mapper extends  Mapper<Text, VectorWritable, IntWritable, VectorWritable> {
		public static KDTreeForest_byte kdtf= null;
	//	public static final int dim = 128;
		public static int K = 0; //number of clusters 
		public static String clusters_in= null; // input folder of clusters
		
		public static byte[][] varray = null; // clusters array
		public static float[] q_vector = new float[AKM_byte.dim];
		public static double[] q_vector_double = new double[AKM_byte.dim];
...
//			String features = conf.get("features");
	...
	public static class AKM_Reducer extends  Reducer<IntWritable, VectorWritable, IntWritable, VectorWritable> {
		...
		static byte[][] old_clusters = null; // clusters array
		...
		public void reduce(IntWritable key, Iterable<VectorWritable> values, Context context) 
			...
			try {
				if(getDistance(new_cluster_float, old_clusters[key.get()]) > cd){
					isConverged = false;
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
		...
		//get the Euclidean distance of two vectors of float and byte
		static float getDistance(float[] v1, byte[] v2) throws Exception {
			// TODO Auto-generated method stub
			if(v1 == null || v2 == null){
	...
	private void eliminate_empty_clusters(String clusters_out,	String clusters_in) 
		...
		if(empty_clusterIds.size() == 0){
			...
			byte[][] old_clusters =getClustersFromPath( conf, clusters_in, cluster_num) ;
			//
	...
	private void getFinalResult(Configuration conf, String inputfolder,	String outputfile, String features_folder) 
		...
		byte[][] clusters = getClustersFromPath(conf, inputfolder, cluster_num);
	...
	private static byte[][] getClustersFromPath(Configuration conf, String clusters_in2, int k2) 
			throws IOException, InstantiationException, IllegalAccessException {
		// TODO Auto-generated method stub
		byte[][] dataset = new byte[k2][dim];
		...
		for(String file : files){
			...
			while(reader.next(key, value)){
				...
				//
				dataset[index] = new byte[128];
				Vector v = value.get();
				for(int i = 0; i < 128; i ++){
					dataset[index][i] = (byte) v.get(i);
				}
			}
			reader.close();
		}
